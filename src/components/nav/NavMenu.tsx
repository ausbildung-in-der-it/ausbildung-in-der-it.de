    ...
    <div className="hidden md:flex items-center">
      <ul className="flex h-14 items-center" role="menubar">
        ...
        {visibleNavItems.map((item) => (
          <li key={item.title} role="none">
            {item.megaMenu && (
              <NavMenuButton
                title={item.title}
                isOpen={openMegaMenu === item.megaMenu.id}
                menuId={item.megaMenu.id}
                onClick={() => setOpenMegaMenu(openMegaMenu === item.megaMenu.id ? null : item.megaMenu.id)}
              />
            )}
          </li>
        ))}
        ...
        {navItems.slice(3).map((item) => (
          <li key={item.title} role="none" className="hidden xl:block">
            {item.megaMenu && (
              <NavMenuButton
                title={item.title}
                isOpen={openMegaMenu === item.megaMenu.id}
